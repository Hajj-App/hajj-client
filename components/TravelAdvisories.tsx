...
const TravelAdvisories = React.memo(() => {
  ...
  const renderAdvisory = useCallback((advisory: Advisory) => (
    <View key={advisory.id} style={styles.advisoryItem}>
      <View style={styles.advisoryHeader}>
        <Text style={styles.advisoryTitle} numberOfLines={1}>{advisory.title}</Text>
        <Text style={styles.advisoryDate}>{advisory.date}</Text>
      </View>
      <Text style={styles.advisoryDescription} numberOfLines={3}>{advisory.description}</Text>
